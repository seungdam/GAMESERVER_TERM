#include <windows.h>  
#include <iostream>  
#include "OBDCManager.h"

void OBDCManager::AllocateHandles()
{
...
void OBDCManager::ConnectDataSource(const SQLWCHAR* obdc)
{
    SQLSetConnectAttr(hdbc, SQL_LOGIN_TIMEOUT, (SQLPOINTER)5, 0);
    retcode = SQLConnect(hdbc, (SQLWCHAR*)obdc, SQL_NTS, NULL, 0, NULL, 0);
}

void OBDCManager::ExecuteStatementDirect(const SQLWCHAR* sql)
{
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
    {
...
void OBDCManager::ExecuteStatement(const SQLWCHAR* exec)
{
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
    {
        retcode = SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
...
void OBDCManager::RetrieveResult()
{
...
void OBDCManager::DisconnectDataSource()
{
    if (hstmt)
    {
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
...
void OBDCManager::show_error(SQLHANDLE hHandle, SQLSMALLINT hType, RETCODE RetCode)
{
    SQLSMALLINT iRec = 0;
    SQLINTEGER iError;
